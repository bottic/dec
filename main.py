...
from logger_dec import _logger, logger
...
def phones(list):
    ...
    for element in list:
        ...
        if len(re.findall(r'[а-яёА-ЯЁ.]+', phone_number)) > 0:
            phone_number = re.sub(
                r'(\+7|8)\s*\(?(\d{3})[\)\s-]*(\d{3})[-]?(\d{2})[-]?(\d{2})\s\(?([а-яёА-ЯЁ.]+)\s(\d+)',
                r'7(\2)\3-\4-\5 \6\7', phone_number)
            # print(phone_number)
        else:
            phone_number = re.sub(r'(\+7|8)\s*\(?(\d{3})[\)\s-]*(\d{3})[-]?(\d{2})[-]?(\d{2})', r'+7(\2)\3-\4-\5',
                                  phone_number)
            # print(phone_number)
